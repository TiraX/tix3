...
#define BGR8_TO_RGBA8(pSrc, pDst)	{\
	...
	TImagePtr TImage::LoadImageTGA(TFile& FileInput, int32* PixelDepth)
	{
		...
		switch (header.PixelDepth)
		{
			...
			default:
			{
				_LOG(ELog::Error, "Unsupported TGA format. [%s]\n", FileInput.GetFileName().c_str());
				return NULL;
			}
		}
		...
		{
			_LOG(ELog::Error, "Unsupported TGA file type. [%s]\n", FileInput.GetFileName().c_str());
			return NULL;
		}
		...
		{
			RuntimeFail();
			_LOG(ELog::Error, "Compressed TGA format is not supported yet.");
		}
	...
	FInt2 TImage::LoadImageTGADimension(const TString& TgaName)
	{
		TFile ImageFile;
		if (ImageFile.Open(TgaName, EFA_READ))
		{
			...
			return FInt2(header.ImageWidth, header.ImageHeight);
		}
		return FInt2();
	}
