...
namespace tix
{
	...
	TImage* DecompressDXT1(TResTextureDefine* Texture) 
	{
		...
		if (BlockSize == 0)
		{
			_LOG(ELog::Error, "Invalid block size.\n");
			return nullptr;
		}

		if (Texture->Desc.Type != ETT_TEXTURE_2D)
		{
			_LOG(ELog::Error, "Not support DXT1 format other than Texture2D.");
			return nullptr;
		}
	...
	TImage* DecompressDXT5(TResTextureDefine* Texture)
	{
		...
		if (Texture->Desc.Type != ETT_TEXTURE_2D)
		{
			_LOG(ELog::Error, "Not support DXT5 format other than Texture2D.");
			return nullptr;
		}
	...
	TImage* DecodeDXT(TResTextureDefine* Texture)
	{
		...
		else if (Texture->Desc.Format == EPF_A8)
		{
			if (Texture->Desc.Type != ETT_TEXTURE_2D)
			{
				_LOG(ELog::Error, "Not support EPF_A8 format other than Texture2D.");
				return nullptr;
			}
		...
		{
			TI_ASSERT(0);
			_LOG(ELog::Error, "Unsupported dds format.\n");
		}
