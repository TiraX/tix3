...
FPackedView CreatePackedView(const FPackedViewParams& Params)
{
	...
	const FFloat4 ViewSizeAndInvSize(ViewRect.GetWidth(), ViewRect.GetHeight(), 1.0f / float(ViewRect.GetWidth()), 1.0f / float(ViewRect.GetHeight()));

	FPackedView PackedView;
	PackedView.TranslatedWorldToView = Params.ViewInfo.MatView;
	PackedView.TranslatedWorldToView.SetTranslation(FFloat3());
		//FMat4(Params.ViewMatrices.GetOverriddenTranslatedViewMatrix());	// LWC_TODO: Precision loss? (and below)
	...
	PackedView.ViewForwardAndNearPlane = FFloat4((FFloat3)Params.ViewMatrices.GetOverriddenTranslatedViewMatrix().GetColumn(2), Params.ViewMatrices.ComputeNearPlane());
